...
static void SPI_TxBuffer(uint8_t *buffer, uint16_t len) {
  // full execution of below function
...
/* SPI transmit buffer */
static void SPI_TxBuffer_DMA(uint8_t *buffer, uint16_t len) {
  if (spiDmaTransferComplete ==
      1) {                      // do nothing while transaction is incomplete
    spiDmaTransferComplete = 0; // should come before as we cannot guarantee
    // full execution of below function
    HAL_SPI_Transmit_DMA(HSPI_SDCARD, buffer, len);
  }
}

static HAL_StatusTypeDef SPI_RxBuffer_DMA(uint8_t *buffer, uint16_t len) {
  uint8_t dummy[len];
  ...
  HAL_StatusTypeDef st =
      HAL_SPI_TransmitReceive_DMA(HSPI_SDCARD, dummy, buffer, len);
